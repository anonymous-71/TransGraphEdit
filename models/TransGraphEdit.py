...
from models.encoder import MPNEncoder, ContraNorm, MultiHeadGraphAttention
from models.model_utils import (creat_edits_feats, index_select_ND,
                                unbatch_feats)


class TransGraphEdit(nn.Module):
    def __init__(self,
                 config: Dict,
                 atom_vocab: Vocab,
                 bond_vocab: Vocab,
        ...
        """
        super(TransGraphEdit, self).__init__()
    ...
    def _build_layers(self) -> None:
        ...
        self.attn = MultiHeadGraphAttention(
            in_dim=self.config['mpn_size'],
            hidden_dim=self.config['mpn_size'],
            num_heads=self.config['n_heads']
        )
        # if config['use_attn']:
        #     self.attn = MultiHeadGraphAttention(
        #         in_dim=config['mpn_size'], hidden_dim=config['mpn_size'], num_heads=config['n_heads'])
    ...
    def compute_edit_scores(self, prod_tensors: Tuple[torch.Tensor],
        ...
        if self.config['use_attn']:
            ...
            # print(f"Number of unpacked attributes: {len(self.attn(feats, mask))}")
            # attention_score, feats = self.attn(feats, mask)
            output = self.attn(feats, mask)
    ...
    def predict(self, prod_smi: str, rxn_class: int = None, max_steps: int = 9):
        ...
        while not done and steps <= max_steps:
            ...
            # Extract bond logits from edit_logits
            bond_logits = edit_logits[0][:max_bond_idx]
            bond_logits = bond_logits.reshape(products.GetNumBonds(), self.bond_outdim)

            # Find indices for all variations of 'Delete Bond'
            delete_bond_indices = [idx for elem, idx in self.bond_vocab.elem_to_idx.items() if 'Delete Bond' in elem]

            # Apply a high score to all 'Delete Bond' variations in bond logits
            for delete_bond_idx in delete_bond_indices:
                bond_logits[:, delete_bond_idx] += 5

            idx = torch.argmax(edit_logits[0])
            val = edit_logits[0][idx]

            # # 强制检查是否有 Delete Bond 操作
            # delete_bond_idx = self.bond_vocab.get_index('Delete Bond')
            # if delete_bond_idx in torch.argsort(edit_logits[0])[-1]:  # 取出 top 的值
            #     idx = delete_bond_idx  # 强制执行 Delete Bond
