...
def apply_edit_to_mol(mol: Chem.Mol, edit: Tuple, edit_atom: Any) -> Chem.Mol:
    ...
    try:
        ...
        elif edit[0] == 'Add Bond':
            if edit_atom[0] != edit_atom[1]:  # 检查两个原子是否不同
                edit_exe = BondEditAction(*edit_atom, *edit[1], action_vocab='Add Bond')
                new_mol = edit_exe.apply(mol)
            else:
                raise ValueError("Cannot add a bond between the same atom.")
...
def process_batch(batch_graphs, args):
    if not batch_graphs:
        raise ValueError("batch_graphs is empty, no valid-1 graph sequences in this batch.")
    ...
    for idx, rxn_data in enumerate(rxns_data):
        ...
        for i, edit in enumerate(rxn_data.edits):
            if int_mol is None:
                ...
                break
            if edit == 'Terminate':
                graph = RxnGraph(prod_mol=Chem.Mol(int_mol), edit_to_apply=edit, reac_mol=Chem.Mol(r_mol), rxn_class=rxn_data.rxn_class, use_rxn_class=args.use_rxn_class)
                ...
                graph = RxnGraph(prod_mol=Chem.Mol(int_mol), edit_to_apply=edit, edit_atom=rxn_data.edits_atom[i], reac_mol=Chem.Mol(r_mol), rxn_class=rxn_data.rxn_class, use_rxn_class=args.use_rxn_class)
                graph_seq.append(graph)
                int_mol = apply_edit_to_mol(Chem.Mol(int_mol), edit, rxn_data.edits_atom[i])
                if int_mol is None:
                    print(f"Skipping invalid edit at reaction {idx}, step {i}")
                    break

        if len(graph_seq) == 0 or final_smi is None:
            print(f"No valid-1 states found. Skipping reaction {idx}")
            print()
            sys.stdout.flush()
            continue
