...
def prepare_edit_labels(graph_batch: List[MolGraph], edits: List[Any], edit_atoms: List[Any], bond_vocab: List,
                        atom_vocab: List) -> torch.tensor:
    """
    ...
    for prod_graph, edit, edit_atom in zip(graph_batch, edits, edit_atoms):
        ...
        if edit == 'Terminate':
            ...
            bond = prod_graph.mol.GetBondBetweenAtoms(a_start, a_end)
            # 在获取键之前添加额外的检查
            if a_start != a_end:  # 确保两个原子不相同
                bond = prod_graph.mol.GetBondBetweenAtoms(a_start, a_end)
                if bond is not None:
                    b_idx = bond.GetIdx()
                    edit_idx = bond_vocab.get_index(edit)
                    ...
                    print(f"Warning: No bond found between atoms {a_start} and {a_end}")
            else:
                print(f"Warning: Attempted to get bond between the same atom: {a_start}")
