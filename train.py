...
from models import TransGraphEdit
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(valid_data):
            ...
            for idx, prod_smi in enumerate(prod_smi_batch):
                ...
                if len(edits) == 0 or len(edits_atom) == 0:
                    print(f"Warning: No edits returned for product {prod_smi}")
                    continue
                if idx >= len(edits_batch) or idx >= len(edits_atom_batch):
                    print(f"Warning: Index {idx} out of range for batch data")
                    continue

                if edits == edits_batch[idx] and edits_atom == edits_atom_batch[idx]:
                    total_accuracy += 1.0
                if len(edits) > 0 and len(edits_atom) > 0 and \
                        len(edits_batch[idx]) > 0 and len(edits_atom_batch[idx]) > 0 and \
                        edits[0] == edits_batch[idx][0] and edits_atom[0] == edits_atom_batch[idx][0]:
...
def main(args):
    ...
    model = TransGraphEdit(config=model_config, atom_vocab=atom_vocab,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='C-H Arylation',
                        help='dataset: C-H Arylation or uspto_50k')
    ...
    parser.add_argument('--mpn_size', type=int,
                        default=128, help='MPN hidden_dim')
    ...
    parser.add_argument('--mlp_size', type=int,
                        default=128, help='MLP hidden_dim')
